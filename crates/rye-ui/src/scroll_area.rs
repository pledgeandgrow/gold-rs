...
impl Default for ScrollAreaProps {
    fn default() -> Self {
        Self { height: "300px".to_string(), width: None,
               scrollbar_color: vars::TEXT_SUBTLE.to_string(), scrollbar_width: "8px".to_string(),
...
impl ScrollAreaProps {
    ...
    pub fn scrollbar_color(mut self, c: impl Into<String>) -> Self { self.scrollbar_color = c.into(); self }
    pub fn scrollbar_width(mut self, w: impl Into<String>) -> Self { self.scrollbar_width = w.into(); self }
}
    ...
    pub fn render(props: ScrollAreaProps) -> Element {
        ...
        Element::Template(Template::new_element("div",
            vec![("style".to_string(), style),
                 ("class".to_string(), format!("rye-scroll-area {}", props.class.as_deref().unwrap_or(""))),
                 ("data-scrollbar-color".to_string(), props.scrollbar_color.clone()),
                 ("data-scrollbar-width".to_string(), props.scrollbar_width.clone())],
            Vec::new(), Vec::new()))
    }
}
...
mod tests {
    ...
    fn test_scroll_area_builder() {
        let p = ScrollAreaProps::default().height("500px").width("400px").scrollbar_color("#2563eb");

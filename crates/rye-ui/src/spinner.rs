...
impl Default for SpinnerProps {
    fn default() -> Self {
        Self { size: Size::Medium, color: vars::PRIMARY.to_string(), label: None, class: None, style: None }
    }
}
...
impl Spinner {
    pub fn render(props: SpinnerProps) -> Element {
        let dim = match props.size {
            Size::Small => "16px", Size::Medium => "24px", Size::Large => "36px",
        };
        let border_width = match props.size {
            Size::Small => "2px", Size::Medium => "3px", Size::Large => "4px",
        };
        let spinner_style = format!(
            ...
            dim, dim, border_width, vars::BORDER, props.color,
            props.style.as_deref().unwrap_or(""),
        );

        let mut children = vec![Template::new_element("div",
            vec![("class".to_string(), format!("rye-spinner {}", props.class.as_deref().unwrap_or(""))),
                 ("style".to_string(), spinner_style)],
            Vec::new(), Vec::new())];

        if let Some(label) = &props.label {
            children.push(Template::new_element("span",
                vec![("style".to_string(), format!("font-size:var(--rye-font-size-md);color:{};", vars::TEXT_MUTED))],
                Vec::new(), vec![Template::text(label)]));
        }

        Element::Template(Template::new_element("div",
            vec![("style".to_string(), "display:inline-flex;align-items:center;gap:8px;".to_string())],
            Vec::new(), children))
    }
}
...
mod tests {
    ...
    fn test_spinner_builder() {
        let p = SpinnerProps::default().size(Size::Large).color("#dc2626").label("Loading...");

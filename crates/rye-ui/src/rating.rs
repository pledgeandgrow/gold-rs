...
impl RatingProps {
    ...
    pub fn allow_half(mut self, a: bool) -> Self {
        self.allow_half = a;
        self
    }
}
...
impl Rating {
    pub fn render(props: RatingProps) -> Element {
        ...
        let stars: Vec<Template> = (0..props.max)
            .map(|i| {
                let filled = i < props.value;
                let star_color = if filled {
                    props.color.clone()
                } else {
                    vars::BORDER.to_string()
                };
        ...
        if props.value > 0 {
            children.push(Template::new_element(
                ...
                vec![(
                    "style".to_string(),
                    format!(
                        "margin-left:8px;font-size:var(--rye-font-size-md);color:{};",
                        vars::TEXT_MUTED
                    ),
                )],
                Vec::new(),
                vec![Template::text(&format!("{}/{}", props.value, props.max))],
            ));
        }
        ...
        Element::Template(Template::new_element(
            ...
            vec![
                ...
                (
                    "class".to_string(),
                    format!("rye-rating {}", props.class.as_deref().unwrap_or("")),
                ),
            ],
            Vec::new(),
            children,

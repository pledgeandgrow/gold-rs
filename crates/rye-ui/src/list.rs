...
impl Default for ListProps {
    fn default() -> Self {
        Self { items: Vec::new(), variant: ListVariant::Unordered, class: None, style: None }
    }
}

impl ListProps {
    pub fn items(mut self, i: Vec<ListItem>) -> Self { self.items = i; self }
    pub fn variant(mut self, v: ListVariant) -> Self { self.variant = v; self }
}
...
impl List {
    pub fn render(props: ListProps) -> Element {
        let items: Vec<Template> = props.items.iter().map(|item| {
            Template::new_element("li",
                vec![("style".to_string(), format!("margin-bottom:4px;font-size:var(--rye-font-size-md);color:{};", vars::TEXT)),
                     ("class".to_string(), "rye-list-item".to_string())],
                Vec::new(), vec![Template::text(&item.text)])
        }).collect();

        let style = format!("{}{}", props.variant.style(), props.style.as_deref().unwrap_or(""));

        Element::Template(Template::new_element(props.variant.tag(),
            vec![("class".to_string(), format!("rye-list {}", props.class.as_deref().unwrap_or(""))),
...
mod tests {
    ...
    #[test]
    fn test_list_render() {
        let el = List::render(ListProps::default()

...
impl TreeNode {
    pub fn new(label: impl Into<String>) -> Self {
        Self { label: label.into(), icon: None, expanded: false, children: Vec::new() }
    }
...
impl Default for TreeViewProps {
    fn default() -> Self { Self { root: TreeNode::new("Root"), class: None, style: None } }
}

impl TreeViewProps {
    pub fn root(mut self, r: TreeNode) -> Self { self.root = r; self }
}
...
impl TreeView {
    pub fn render(props: TreeViewProps) -> Element {
        ...
        let style = format!("font-size:14px;color:#1e293b;user-select:none;{}", props.style.as_deref().unwrap_or(""));

        Element::Template(Template::new_element("div",
            vec![("style".to_string(), style),
                 ("class".to_string(), format!("rye-tree-view {}", props.class.as_deref().unwrap_or("")))],
            Vec::new(), vec![node]))
    }
}
...
fn render_node(node: &TreeNode, depth: usize) -> Template {
    ...
    let mut row_children = vec![
        Template::new_element("span",
            vec![("style".to_string(), "font-size:10px;color:#64748b;width:12px;".to_string())],
            Vec::new(), vec![Template::text(arrow)]),
    ];

    if let Some(icon) = &node.icon {
        row_children.push(Template::new_element("span",
    ...
    let mut children = vec![Template::new_element("div",
        vec![("style".to_string(), row_style), ("class".to_string(), "rye-tree-node".to_string())],
        Vec::new(), row_children)];
...
mod tests {
    ...
    fn test_tree_node_builder() {
        let n = TreeNode::new("src").icon("📁").expanded()
            .children(vec![TreeNode::new("main.rs").icon("📄"), TreeNode::new("lib.rs").icon("📄")]);
        assert!(n.expanded);
    ...
    fn test_tree_view_render() {
        let el = TreeView::render(TreeViewProps::default()
            .root(TreeNode::new("Project").expanded()
                .children(vec![

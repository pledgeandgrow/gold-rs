...
impl BreadcrumbProps {
    pub fn items(mut self, i: Vec<BreadcrumbItem>) -> Self {
        self.items = i;
        self
    }
    ...
    pub fn render(props: BreadcrumbProps) -> Element {
        ...
        for (i, item) in props.items.iter().enumerate() {
            let style = if item.current {
                ...
                format!(
                    "font-size:var(--rye-font-size-md);color:{};cursor:pointer;",
                    vars::PRIMARY
                )
            };
            ...
            let mut attrs = vec![
                ...
                ("class".to_string(), "rye-breadcrumb-item".to_string()),
            ];
            ...
            let tag = if item.href.is_some() && !item.current {
                "a"
            } else {
                "span"
            };
        ...
        Element::Template(Template::new_element(
            ...
            vec![
                (
                    "class".to_string(),
                    format!("rye-breadcrumb {}", props.class.as_deref().unwrap_or("")),
                ),
                (
                    "style".to_string(),
...
mod tests {
    ...
    fn test_breadcrumb_render() {
        let el = Breadcrumb::render(BreadcrumbProps::default().items(vec![

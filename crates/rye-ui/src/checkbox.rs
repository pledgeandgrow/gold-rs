...
impl Default for CheckboxProps {
    fn default() -> Self {
        Self { label: None, checked: false, indeterminate: false, disabled: false, class: None, style: None }
    }
}
...
impl CheckboxProps {
    ...
    pub fn indeterminate(mut self, i: bool) -> Self { self.indeterminate = i; self }
...
impl Checkbox {
    pub fn render(props: CheckboxProps) -> Element {
        ...
        let mut input_attrs = vec![
            ...
            ("class".to_string(), format!("rye-checkbox {}", props.class.as_deref().unwrap_or(""))),
        ];
        if props.checked {
        ...
        let mut children = vec![Template::new_element("input", input_attrs, Vec::new(), Vec::new())];

        if let Some(label) = &props.label {
            children.push(Template::text(label));
        }

        let style = if let Some(extra) = &props.style { format!("{}{}", label_style, extra) } else { label_style };
        Element::Template(Template::new_element("label",
            vec![("style".to_string(), style)], Vec::new(), children))
    }
}
...
mod tests {
    ...
    fn test_checkbox_builder() {
        let p = CheckboxProps::default().label("Accept terms").checked(true).disabled(true);

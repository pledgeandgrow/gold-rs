...
impl Default for HoverCardProps {
    fn default() -> Self {
        Self { trigger: String::new(), title: String::new(), description: String::new(),
               image: None, width: "280px".to_string(), class: None, style: None }
    }
}
...
impl HoverCard {
    pub fn render(props: HoverCardProps) -> Element {
        ...
        let trigger_style = format!("color:{};cursor:pointer;text-decoration:underline;font-size:var(--rye-font-size-md);", vars::PRIMARY);
        ...
        if let Some(img) = &props.image {
            card_children.push(Template::new_element("img",
                vec![("src".to_string(), img.clone()),
                     ("style".to_string(), "width:48px;height:48px;border-radius:50%;margin-bottom:8px;".to_string())],
                Vec::new(), Vec::new()));
        }
        ...
        card_children.push(Template::new_element("div",
            ...
            Vec::new(), vec![Template::text(&props.description)]));

        let children = vec![
            Template::new_element("span",
                vec![("style".to_string(), trigger_style.to_string()),
                     ("class".to_string(), "rye-hover-card-trigger".to_string())],
                Vec::new(), vec![Template::text(&props.trigger)]),
            Template::new_element("div",
                vec![("style".to_string(), card_style),
                     ("class".to_string(), format!("rye-hover-card {}", props.class.as_deref().unwrap_or("")))],
                Vec::new(), card_children),
        ];
...
mod tests {
    ...
    fn test_hover_card_render() {
        let el = HoverCard::render(HoverCardProps::default()

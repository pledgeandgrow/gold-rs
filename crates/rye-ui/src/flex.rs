...
impl FlexDirection {
    pub fn as_str(&self) -> &'static str {
        match self {
            ...
            Self::RowReverse => "row-reverse", Self::ColumnReverse => "column-reverse",
        }
    }
}
...
impl FlexWrap {
    pub fn as_str(&self) -> &'static str {
        match self {
            Self::NoWrap => "nowrap", Self::Wrap => "wrap", Self::WrapReverse => "wrap-reverse",
        }
    }
}
...
impl JustifyContent {
    pub fn as_str(&self) -> &'static str {
        match self {
            Self::FlexStart => "flex-start", Self::FlexEnd => "flex-end", Self::Center => "center",
            Self::SpaceBetween => "space-between", Self::SpaceAround => "space-around", Self::SpaceEvenly => "space-evenly",
        }
    }
}
...
impl AlignItems {
    pub fn as_str(&self) -> &'static str {
        match self {
            Self::Stretch => "stretch", Self::FlexStart => "flex-start", Self::FlexEnd => "flex-end",
            Self::Center => "center", Self::Baseline => "baseline",
        }
    }
}
...
impl Default for FlexProps {
    fn default() -> Self {
        Self { direction: FlexDirection::Row, wrap: FlexWrap::NoWrap,
               justify: JustifyContent::FlexStart, align: AlignItems::Stretch,
               gap: None, class: None, style: None }
    }
}

impl FlexProps {
    pub fn direction(mut self, d: FlexDirection) -> Self { self.direction = d; self }
    pub fn wrap(mut self, w: FlexWrap) -> Self { self.wrap = w; self }
    ...
    pub fn gap(mut self, g: impl Into<String>) -> Self { self.gap = Some(g.into()); self }
}
...
impl Flex {
    pub fn render(props: FlexProps) -> Element {
        let mut parts = vec![
            format!("display:flex;flex-direction:{};flex-wrap:{};justify-content:{};align-items:{}",
                props.direction.as_str(), props.wrap.as_str(), props.justify.as_str(), props.align.as_str()),
        ];
        if let Some(g) = &props.gap { parts.push(format!("gap:{}", g)); }
        if let Some(s) = &props.style { parts.push(s.clone()); }
        let style = parts.join(";");

        Element::Template(Template::new_element("div",
            vec![("class".to_string(), format!("rye-flex {}", props.class.as_deref().unwrap_or(""))),
...
mod tests {
    ...
    #[test]
    fn test_flex_render() {
        let el = Flex::render(FlexProps::default().direction(FlexDirection::Column).gap("8px"));

...
impl Default for CircularProgressProps {
    fn default() -> Self {
        Self { value: None, size: 48, stroke_width: 4,
               color: vars::PRIMARY.to_string(), track_color: vars::BORDER.to_string(),
               label: None, show_percentage: false, class: None, style: None }
    }
}
...
impl CircularProgressProps {
    ...
    pub fn stroke_width(mut self, w: u32) -> Self { self.stroke_width = w; self }
    ...
    pub fn show_percentage(mut self, s: bool) -> Self { self.show_percentage = s; self }
}
...
impl CircularProgress {
    pub fn render(props: CircularProgressProps) -> Element {
        ...
        let (dash_offset, animation) = match props.value {
            ...
            None => (circumference.to_string(), "rye-circular-spin 1s linear infinite"),
        };
        ...
        let mut children = vec![
            Template::new_element("svg",
                vec![("width".to_string(), props.size.to_string()),
                     ("height".to_string(), props.size.to_string()),
                     ("viewBox".to_string(), format!("0 0 {} {}", props.size, props.size)),
                     ("style".to_string(), svg_style),
                     ("class".to_string(), "rye-circular-progress-svg".to_string())],
                Vec::new(), vec![
                    Template::new_element("circle",
                        vec![("cx".to_string(), (props.size / 2).to_string()),
                    ...
                    Template::new_element("circle",
                        vec![("cx".to_string(), (props.size / 2).to_string()),
                             ...
                             ("transform".to_string(), format!("rotate(-90 {} {})", props.size / 2, props.size / 2))],
                        Vec::new(), Vec::new()),
                ]),
        ];
        ...
        if props.show_percentage {
            ...
            children.push(Template::new_element("span",
                vec![("style".to_string(), format!("position:absolute;font-size:var(--rye-font-size-xs);color:{};", vars::TEXT_MUTED))],
                Vec::new(), vec![Template::text(label)]));
        }

        Element::Template(Template::new_element("div",
            vec![("style".to_string(), container_style),
                 ("class".to_string(), format!("rye-circular-progress {}", props.class.as_deref().unwrap_or("")))],
            Vec::new(), children))
    }
}
...
mod tests {
    ...
    fn test_circular_progress_builder() {
        let p = CircularProgressProps::default().value(75.0).size(64).color("#16a34a").show_percentage(true);
    ...
    fn test_circular_progress_render_determinate() {
        let el = CircularProgress::render(CircularProgressProps::default().value(60.0).show_percentage(true));

...
impl Default for VirtualListProps {
    fn default() -> Self {
        Self { items: Vec::new(), item_height: 40, visible_height: 400,
               scroll_offset: 0, overscan: 3, class: None, style: None }
    }
}

impl VirtualListProps {
    pub fn items(mut self, i: Vec<VirtualItem>) -> Self { self.items = i; self }
    pub fn item_height(mut self, h: u32) -> Self { self.item_height = h; self }
    pub fn visible_height(mut self, h: u32) -> Self { self.visible_height = h; self }
    pub fn scroll_offset(mut self, o: u32) -> Self { self.scroll_offset = o; self }
    pub fn overscan(mut self, o: u32) -> Self { self.overscan = o; self }
}
...
impl VirtualList {
    pub fn render(props: VirtualListProps) -> Element {
        ...
        let start_idx = (props.scroll_offset / props.item_height).saturating_sub(props.overscan) as usize;
        let visible_count = (props.visible_height / props.item_height + 2 * props.overscan) as usize;
        let end_idx = (start_idx + visible_count).min(props.items.len());

        let visible_items: Vec<Template> = (start_idx..end_idx).map(|i| {
        ...
        let container_style = format!(
            ...
            props.visible_height, props.style.as_deref().unwrap_or(""),
        );

        let inner = Template::new_element("div",
            vec![("style".to_string(), inner_style),
                 ("class".to_string(), "rye-virtual-list-inner".to_string())],
            Vec::new(), visible_items);

        Element::Template(Template::new_element("div",
            vec![("style".to_string(), container_style),
                 ("class".to_string(), format!("rye-virtual-list {}", props.class.as_deref().unwrap_or("")))],
            Vec::new(), vec![inner]))
    }
}
...
mod tests {
    ...
    fn test_virtual_list_builder() {
        let items: Vec<VirtualItem> = (0..100).map(|i| VirtualItem::new(i.to_string(), format!("Item {}", i))).collect();
        let p = VirtualListProps::default().items(items).item_height(50).visible_height(500).scroll_offset(200);
    ...
    fn test_virtual_list_render_with_items() {
        let items: Vec<VirtualItem> = (0..1000).map(|i| VirtualItem::new(i.to_string(), format!("Row {}", i))).collect();
